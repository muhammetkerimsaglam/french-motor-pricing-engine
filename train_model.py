...
def compute_lift_chart(y_true, y_pred, weights, n_bins=10):
    """
    Exposure-agirlikli decile lift chart: tahmin edilen riske gore siralanip
    n_bins dilime bolunur, her dilimde gercek vs tahmin edilen ortalama
    frekans karsilastirilir. Model kaliteli ise dilimler arttikca hem
    gercek hem tahmin edilen deger monotonik artmali (ayirt edici guc).
    """
    df = pd.DataFrame({"actual": y_true, "predicted": y_pred, "weight": weights})
    df = df.sort_values("predicted").reset_index(drop=True)
    df["cum_weight"] = df["weight"].cumsum()
    total_weight = df["weight"].sum()
    # Esit exposure agirlikli dilimler (esit sayida poliçe degil)
    bin_edges = np.linspace(0, total_weight, n_bins + 1)
    df["bin"] = pd.cut(df["cum_weight"], bins=bin_edges, labels=False, include_lowest=True)
    ...
    for b in range(n_bins):
        ...
        rows.append({
            "decile": b + 1,
            "avg_actual": float(np.average(g["actual"], weights=g["weight"])),
            "avg_predicted": float(np.average(g["predicted"], weights=g["weight"])),
            "exposure": float(g["weight"].sum()),
        })
    return rows


def compute_gini(y_true, y_pred, weights):
    """
    Sigorta fiyatlamasinda standart olan normalize edilmis Gini katsayisi
    (Frees/Meyers/Cummings yaklasimi): poliçeler tahmin edilen riske gore
    siralanir, gerceklesen hasarlarin kumulatif payi (Lorenz egrisi) ile
    kumulatif exposure paylari karsilastirilir. Gini = 0 -> model ayirt
    edici gucu yok (rastgele siralama). Gini yuksek -> model riskli/riskli
    olmayan poliçeleri iyi ayirt ediyor.
    """
    df = pd.DataFrame({"actual": y_true, "predicted": y_pred, "weight": weights})
    df = df.sort_values("predicted").reset_index(drop=True)
    df["cum_exposure"] = df["weight"].cumsum() / df["weight"].sum()
    weighted_actual = df["actual"] * df["weight"]
    df["cum_actual"] = weighted_actual.cumsum() / weighted_actual.sum()

    x = df["cum_exposure"].values
    y = df["cum_actual"].values
...
def fit_frequency_model(df):
    ...
    gini = compute_gini(y_test, pred_test, w_test)
    lift_rows = compute_lift_chart(y_test, pred_test, w_test, n_bins=10)
    print(f"Test seti Gini katsayısı: {gini:.4f}  (yüksek = model ayırt edici gücü iyi)")
    ...
    return base_frequency, relativities, validation
...
def main():
    ...
    base_freq, freq_relativities, validation = fit_frequency_model(df)
    ...
    print("✅ validation_data.json yazıldı. Faz 3'te app.py bunu lift chart / Gini için okuyacak.")
